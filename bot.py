...
# Состояния
WAITING_CUSTOM_AMOUNT, WAITING_RECEIVE_AMOUNT = range(2)
...
async def custom_amount_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("Нужно положительное число. Попробуй ещё раз:")
        return WAITING_CUSTOM_AMOUNT
...
def main():
    ...
    if BOT_TOKEN == "ВСТАВЬ_СЮДА_ТОКЕН_ОТ_BOTFATHER":
        print("=" * 55)
        print("  СНАЧАЛА ВСТАВЬ ТОКЕН!")
        print("  1. Напиши @BotFather → /newbot")
        print("  2. Скопируй токен")
        print("  3. Вставь его в bot.py в строку BOT_TOKEN")
        print("=" * 55)
        return
